import { i as intern } from './mandatory-setter-Bij6Bx8G.js';
...
class Container {
  ...
  lookup(fullName, options) {
    ...
    (isDevelopingApp() && !(this.registry.isValidFullName(fullName)) && assert('fullName must be a proper full name', this.registry.isValidFullName(fullName)));
  ...
  */
  ...
  factoryFor(fullName) {
    ...
    (isDevelopingApp() && !(this.registry.isValidFullName(normalizedName)) && assert('fullName must be a proper full name', this.registry.isValidFullName(normalizedName)));
...
function factoryFor(container, normalizedName, fullName) {
  ...
  if (isDevelopingApp() && factory && typeof factory._onLookup === 'function') {
...
function destroyDestroyables(container) {
  ...
  for (let key of keys) {
    let value = cache[key];
    (isDevelopingApp() && !(value) && assert('has cached value', value));
    if (value.destroy) {
      value.destroy();
    }
  }
...
class InternalFactoryManager {
  ...
  create(options) {
    ...
    if (isDevelopingApp()) {
      ...
      (isDevelopingApp() && !(typeof this.class.create === 'function') && assert(`Failed to create an instance of '${this.normalizedName}'. Most likely an improperly defined class or an invalid module export.`, typeof this.class.create === 'function'));
    }
...
class Registry {
  ...
  register(fullName, factory, options = {}) {
    (isDevelopingApp() && !(this.isValidFullName(fullName)) && assert('fullName must be a proper full name', this.isValidFullName(fullName)));
    (isDevelopingApp() && !(factory !== undefined) && assert(`Attempting to register an unknown factory: '${fullName}'`, factory !== undefined));
    let normalizedName = this.normalize(fullName);
    (isDevelopingApp() && !(!this._resolveCache[normalizedName]) && assert(`Cannot re-register: '${fullName}', as it has already been resolved.`, !this._resolveCache[normalizedName]));
  ...
  getOption(fullName, optionName) {
    ...
    (isDevelopingApp() && !(type) && assert('has type', type)); // split always will have at least one value
...
if (isDevelopingApp()) {
  ...
  proto.normalizeInjectionsHash = function (hash) {
    ...
    for (let key in hash) {
      if (Object.prototype.hasOwnProperty.call(hash, key)) {
        let value = hash[key];
        (isDevelopingApp() && !(value) && assert('has value', value));
        let {
          specifier
        } = value;
        (isDevelopingApp() && !(this.isValidFullName(specifier)) && assert(`Expected a proper full name, given '${specifier}'`, this.isValidFullName(specifier)));
        injections.push({
          property: key,
          specifier
        });
  ...
  proto.validateInjections = function (injections) {
    ...
    for (let injection of injections) {
      ...
      (isDevelopingApp() && !(this.has(specifier)) && assert(`Attempting to inject an unknown injection: '${specifier}'`, this.has(specifier)));
    }
  };
}
...
function privatize([fullName]) {
  (isDevelopingApp() && !(arguments.length === 1 && fullName) && assert('has a single string argument', arguments.length === 1 && fullName));
...
export { Container as C, INIT_FACTORY as I, Registry as R, getFactoryFor as g, privatize as p, setFactoryFor as s };
