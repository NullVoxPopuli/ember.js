...
APPEND_OPCODES.add(VM_CREATE_COMPONENT_OP, (vm, { op1: flags }) => {
  ...
  // constructor runs, so that addToScope/getScope inside the constructor see
  // the new scope (and its parent chain). Always-on. Mirroring debugRenderTree
  // would be too late: the user constructor runs in manager.create() below.
...
// `getScope()` during their own update see the correct parent chain.
